...
class Adagrad(Optimizer):
    ...
    def update_layer(self, layer: Layer) -> None:
        ...
        layer.weights -= self.learning_rate * layer.weights_gradient / (np.sqrt(layer._m) + 1e-8)
        layer.bias -= self.learning_rate * layer.bias_gradient / (np.sqrt(layer._m_bias) + 1e-8)

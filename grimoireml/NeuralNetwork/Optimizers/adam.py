...
class Adam(Optimizer):
    ...
    def update_layer(self, layer: Layer) -> None:
        ...
        layer._v = self.beta2 * layer._v + (1 - self.beta2) * np.square(layer.weights_gradient)
        ...
        # Bias
        layer._m_bias = self.beta1 * layer._m_bias + (1 - self.beta1) * layer.bias_gradient
        layer._v_bias = self.beta2 * layer._v_bias + (1 - self.beta2) * np.square(layer.bias_gradient)

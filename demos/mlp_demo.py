...
from grimoireml.nn.functions.activation_functions import Sigmoid, ReLU, Softmax
from grimoireml.nn.optimizers import SGD, Adagrad, Adam
from grimoireml.nn.functions.loss_functions import BCE, CCE, MSE
...
def build_model(input_shape: int) -> Sequential:
    ...
    model = Sequential()
    model.add(Dense(input_shape, 128, activation=ReLU()))
    model.add(Dense(128, 64, activation=ReLU()))
    model.add(Dense(64, 32, activation=ReLU()))
    model.add(Dense(32, 10, activation=Sigmoid()))
    model.compile(optimizer=Adam(lr=0.001), loss=CCE(), metrics=[eval_functions.Accuracy(type="multiclass")])
    return model
...
def main():
    ...
    print("GML")
    print(f'epochs: {epochs}, batch_size: {batch_size}, input_shape: {input_shape}')

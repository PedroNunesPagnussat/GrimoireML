...
def test_euclidean_distance_within_range_single_point(x, y, threshold, expected):
    euclidean_distance = EuclideanDistance()
    assert euclidean_distance.within_range(x, y, threshold) == pytest.approx(expected, 0.01)


@pytest.mark.parametrize("x, y, threshold, expected", [
    (np.array([1, 2]), np.array([[1, 2], [1, 2]]), 0, np.array([True, True])),
    (np.array([1, 2]), np.array([[2, 4], [4, 8]]), 6.71, np.array([True, True])),
    (np.array([1, 2]), np.array([[2, 3], [3, 2]]), np.sqrt(2) + 0.1, np.array([True, False])),
    (np.array([4, 5, 6, 7]), np.array([[4, 5, 6, 7], [8, 10, 12, 14]]), np.sqrt(126) - 1, np.array([True, False])),
])

...
@pytest.mark.parametrize("classification_type, y, y_hat, expected", [
    ("binary", np.array([0.49999 , 0.5000001, 0.99999, 0.00001]), np.array([0, 1, 0, 1]), 0.5),
    ("multiclass", np.array([[0.6, 0.1, 0.2, 0.1], [0.2, 0.5, 0.15, 0.15], [0.1, 0.1, 0.5, 0.3], [0.4, 0.05, 0.05, 0.5]]), np.array([0, 1, 2, 2]), 0.75),
    ("multilabel", np.array([[0.3, 0.6], [0.8, 0.2], [0.7, 0.9]]), np.array([[0, 1], [1, 0], [0, 0]]), 2/3),
    ("multilabel", np.array([[0.1, 0.99], [0.56, 0.4], [0.75, 0.25]]), np.array([[0, 1], [1, 0], [0, 0]]), 5/6)
])

def test_accuracy(classification_type, y, y_hat, expected):
    accuracy_func = Accuracy(classification_type)
    assert accuracy_func(y=y, y_hat=y_hat, adjust_y = True) == pytest.approx(expected, 0.01)
